...
public class StartController {
    ...
    public String check() throws Exception {
        URL url = new URL("http://localhost:8081/nats/check/remote");
    ...
    public String start() throws Exception {
        ...
        // test ENABLE_FULL_TRACE false
        works.forEach(WorkBuilder.Work::publish);
        countDownLatch.await();
        return "success";
    }

    @GetMapping(value = "start/remote", produces = MediaType.TEXT_HTML_VALUE)
    ...
    private void requestRemote() {
        try {
            URL url = new URL("http://localhost:8081/nats/start/remote");
            HttpURLConnection connection = (HttpURLConnection) url.openConnection();
            connection.getResponseCode();
            connection.disconnect();
        } catch (Exception e) {
