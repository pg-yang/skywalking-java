...
Skywalking put forward virtual cache concept for analyzing cache related metric ,in order to support this feature ,you should add following tag when developing cache client plugin (e.g. Jedis) 
...
* `cache.type` indicate the cache system name . Usually it's cache system client library name (e.g. Jedis)
* `cache.cmd`  indicate the command what would be sent to cache server (e.g. setnx)
* `cache.op`   indicate the command is used for `write` or `read` operation , usually the value is converting from `command` 
* `cache.key`  indicate the cache key what would be sent to cache server , this tag maybe null ,as string type key would be collected usually.  

In order to decide `op` should be converted from which `command` dynamically , It's better that providing config property .
