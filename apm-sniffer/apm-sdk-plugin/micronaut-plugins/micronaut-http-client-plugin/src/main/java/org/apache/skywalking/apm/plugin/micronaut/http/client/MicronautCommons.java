...
public class MicronautCommons {
    ...
    static void startTrace(MutableHttpRequest<?> request, URI requestURI) {
    ...
    static Publisher<? extends HttpResponse<?>> buildTracePublisher(MutableHttpRequest<?> request, Publisher<? extends HttpResponse<?>> retPublisher) {
        return Flux.from(retPublisher).doOnError(ex -> finishOnAction(request, span -> span.log(ex).errorOccurred()))
                .doOnNext(resp ->
                        finishOnAction(request, span -> {
    ...
    static void finishOnAction(MutableHttpRequest<?> request, Consumer<AbstractSpan> action) {
        try {
