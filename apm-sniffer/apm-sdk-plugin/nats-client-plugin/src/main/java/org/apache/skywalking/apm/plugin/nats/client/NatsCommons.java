...
public class NatsCommons {
    //https://skywalking.apache.org/docs/main/latest/en/concepts-and-designs/event/
    private static final String SID = "sid";
    private static final String REPLY_TO = "reply_to";
    private static final String MSG_STATE = "state";
    private static final String MSG = "message";

    static boolean skipTrace(Message msg) {
        if (msg == null) {
            return true;
        }
        ...
        if (msg.getClass().getName().endsWith("InternalMessage")) {
            return false;
        }
        return msg.getClass().getName().endsWith("ProtocolMessage");
    }

    /*
     * Sometimes , server response StatusMessage , it may be a error message.
     * And We can't trace anything , so we create a local span for recording the status
     */
    static Optional<AbstractSpan> createIncomingSpan(Message message, Connection connection) {
        ...
        addCommonTag(span, message, connection.getConnectedUrl());
    ...
    static void addCommonTag(AbstractSpan span, Message message, String connectionUrl) {
        ...
        SpanLayer.asMQ(span);
        Tags.URL.set(span, connectionUrl);
        if (message.getStatus() != null) {
            ...
            if (StringUtil.isNotBlank(statusMsg)) {
                span.tag(new StringTag(MSG), String.valueOf(statusMsg));
            }
            if (code != 0) {
                span.errorOccurred();
            }
    ...
    static MessageHandler buildTraceMsgHandler(MessageHandler msgHandler, Connection connection) {
        if (msgHandler == null) {
            return null;
        }
        return msg -> {
            ...
            try {
                NatsCommons.createIncomingSpan(msg, connection);
                msgHandler.onMessage(msg);
                ContextManager.stopSpan();
            } catch (Exception e) {
                AbstractSpan span = ContextManager.activeSpan().log(e).errorOccurred();
