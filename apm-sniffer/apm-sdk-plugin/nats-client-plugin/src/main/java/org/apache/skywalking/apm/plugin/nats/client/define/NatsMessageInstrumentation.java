...
/*
 * Nats publish message asynchronously , it put message to local queue , then write message to network by call flush method in another thread .
 * In the step of enqueue , we create a local span ,and create an exit span when flush
 * So We enhance NatsMessage for adding Skywalking dynamic field , used to hold ContextSnapshot in enqueue step
 * See WriterQueueInterceptor
 * See WriterSendMessageBatchInterceptor
 * BTW , ACK  is also done by publishing a message , So we needn't enhance ACK method
 */
    ...
    public InstanceMethodsInterceptPoint[] getInstanceMethodsInterceptPoints() {
        return new InstanceMethodsInterceptPoint[]{
                new InstanceMethodsInterceptPoint() {
                    // Just for Skywalking dynamic field
                    @Override
