...
public class WriterSendMessageBatchInterceptor implements InstanceMethodsAroundInterceptor {
    ...
    public void beforeMethod(EnhancedInstance objInst, Method method, Object[] allArguments, Class<?>[] argumentsTypes, MethodInterceptResult result) throws Throwable {
        ...
        if (skipTrace(message)) {
            return;
        }

        String connection = ((Connection) objInst.getSkyWalkingDynamicField()).getConnectedUrl();

        Object next = message;
        while (next != null) {
            NatsMessage msg = (NatsMessage) next;
            EnhancedInstance enhanced = (EnhancedInstance) next;
            AbstractSpan span = ContextManager.createExitSpan("Nats/Pub/" + message.getSubject(), connection);
            addCommonTag(span, message, connection);
            ...
            LOGGER.info("global traceId -> {}", ContextManager.getGlobalTraceId());
            // clean ,
    ...
    public void handleMethodException(EnhancedInstance objInst, Method method, Object[] allArguments, Class<?>[] argumentsTypes, Throwable t) {
        ...
        try {
            ...
            Object next = message;
            while (next != null) {
                // clean dynamic field as early as possible .
                ((EnhancedInstance) next).setSkyWalkingDynamicField(null);
                next = next(next);
            }
        } catch (IllegalAccessException e) {
            LOGGER.error("next message failure", e);
        }
